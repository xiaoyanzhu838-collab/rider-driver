...
- `11` 和 `21` 虽然都支持正反转
- 但同号速度值映射到机械实际方向时，两者是相反的
- 也就是说：
  - 对 `11` 的“正向”，对应到 `21` 的机械表现会是反方向
  - 对 `21` 的“正向”，对应到 `11` 的机械表现会是反方向

因此后续做双轮联动时，不能把同一个 signed speed 直接同时发给 `11 / 21` 并假设它们会朝同一机械方向转动，必须做方向映射。
...
##### 4.4.4 最终收口结论：不再纠结“理论最大值”，按可用能力交付

后续又做了现场实测核对，最终以实物表现为准，收口结论如下：

- 对 `11 / 21`，我们不再继续追究“理论最大值到底是 `600 / 1023 / 更高` 哪个更准确”。
- 当前最有价值的经验结论是：
  - 输入值越大，`signed speed` 的绝对值越大，实际速度越大。
  - 当输入到 `1023` 时，实测反馈里的 `signed speed` 大约稳定在 `710..720`。
  - 再继续往上加，当前没有看到更明显的提升。
  - 因此很可能在 `1023` 之前，就已经接近这套轮毂节点的有效上限。
- 这组轮子当前只要满足下面几项，就已经足够支撑后续工作：
  - 能控速度
  - 能启停
  - 能正反转
...
- 现场实测补充：输入到 `1023` 时，反馈 `signed speed` 大约在 `710..720`
- 当前不再继续细抠理论最大值，只保留“输入越大，实际越快”的工程结论
- 日常停转优先用 `speed = 0`
